...
var Cache=Class.extend({
 ...
 object._preload=[id,_cache,callback];object.onreadystatechange=function(){ if(this.readyState=="complete"){
 if(this._preload){
...
function grid(cols,rows,colWidth,rowHeight,shuffled){
 ...
 var a=[];for(var y=0;y < rows.length;y++){
...
var Mouse=Class.extend({
 ...
 var m=this._mouse;if(e.touches !==undefined){
 e.preventDefault();}
 m.pressed=true;m._x0=m.x;m._y0=m.y;m.onpress(m);};var eventUp=function(e){
 ...
 m.x=e.touches[0].pageX;m.y=e.touches[0].pageY;}else{
...
var Canvas=Class.extend({
 ...
 var p=document.getElementById(this.id+"_widgets");if(p)p.parentNode.removeChild(p);this._widgets=[];this.variables=[];},
 focus: function(){
 _ctx=this._ctx;
 },
...
try{
 var s=screen;var p=[s.width,s.height,s.availWidth,s.availHeight,s.colorDepth,s.pixelDepth];screen=screen_;screen.width=p[0];screen.height=p[1];screen.availWidth=p[2];screen.availHeight=p[3];screen.colorDepth=p[4];screen.pixelDepth=p[5];}catch(e){
}
...
function widget(canvas,variable,type,options){
 ...
 var parent=(o && o.parent)? o.parent : document.getElementById(canvas.id+"_widgets");if(!parent){
