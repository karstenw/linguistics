...
class Word:
    ...
    def __repr__(self):
        ...
        if ReadableRepresentations:
            return str(self)
        return "getWord(%s, %s)" % ( repr(self.form), repr(self.pos) )
...
class Synset:
    ...
    def __init__(self, pos, offset, line):
        ...
        tokenint = int(tokens[3], 16)
        (self._senseTuples, remainder) = _partition(tokens[4:], 2,  tokenint )
    ...
    def __repr__(self):
        ...
        if ReadableRepresentations:
            return str(self)
        return "getSynset(%s, %s)" + ( repr(self.pos), repr(self.offset) )
...
def setupLexnames():
    f = io.open(WNSEARCHDIR+'/lexnames', 'r', encoding="utf-8")
...
class Dictionary:
    ...
    def __init__(self, pos, filenameroot):
        ...
        self.dataFile = io.open(_dataFilePathname(filenameroot), 'r', encoding="utf-8")
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, str):
            ...
            return self.getWord( firstword )
            # return self.getWord(string.replace( line[:firstspace], '_', ' '), line)
        else:
    ...
    def _testKeys(self):
        ...
        print( "Testing: ", self )
        file = open(self.indexFile.file.name, 'r', encoding="utf-8")
        counter = 0
        while 1:
            line = file.readline()
            if line == '':
...
class _IndexFile:
    ...
    def __init__(self, pos, filenameroot):
        # pdb.set_trace()
        path = _indexFilePathname(filenameroot)
